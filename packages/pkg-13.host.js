// pkg-13 host: MiniMax / MiniMaxi Token Plan coding_plan/remains fetcher.
// Returns a JSON snapshot for the Client half to render.
//
// Strategy:
// - Try both regional endpoints (.io and .cn) and every known credential ref.
// - Use current_interval_remaining_percent directly when total_count is 0
//   (minimaxi.com returns 0 quota counts but provides a direct percentage).
// - Fall back to (total - remaining) / total arithmetic otherwise.
// - Cache the first successful read for 5 minutes per (apiKey, endpoint).
// - All diagnostic details are written to the DSH plugin log under
//   the [token-plan-window] tag — the Client UI never exposes them inline.
...
function deriveUsedPct(m) {
  const remainingPct = pickNum(m, [
    'current_interval_remaining_percent',
    'remaining_percent',
    'remainingPercent',
  ])
  const total = pickNum(m, [
    'current_interval_total_count',
    'total_count',
    'total',
    'total_quota',
  ])
  const remaining = pickNum(m, [
...
function parseRemains(bodyText) {
  ...
  if (payload.status_code && payload.status_code !== 0) {
    return {
      ...
      message: payload.status_msg || 'invalid api key',
    }
  }
  ...
  for (let i = 0; i < ALL_KEYS.length; i += 1) {
    ...
    try {
      ...
      if (resolved && resolved.value) {
        found.push({ name: k, value: resolved.value, source: resolved.source || null })
      }
    } catch (err) {}
  }
  return found
...
function logDiagnostics(tag, payload) {
  try {
    console.log('[token-plan-window] ' + tag + ' ' + JSON.stringify(payload, null, 2))
  } catch (err) {
    try {
      console.log('[token-plan-window] ' + tag + ' (failed to stringify payload)')
    } catch (err2) {}
  }
}
  ...
  apply(ctx) {
    harness.handle('read-token-meter', async () => {
      try {
        ...
        if (!credentials) {
          logDiagnostics('no-credentials', { provider, model })
          return { ok: false, reason: 'no-credentials' }
        }

        const keys = await collectCandidateKeys(credentials)
        if (keys.length === 0) {
          logDiagnostics('no-api-key', { provider, model, tried: ALL_KEYS })
          return { ok: false, reason: 'no-api-key' }
        }

        const attempts = []
        const endpoints = [ENDPOINT_CN, ENDPOINT_IO]
        for (let e = 0; e < endpoints.length; e += 1) {
          ...
          for (let k = 0; k < keys.length; k += 1) {
            ...
            let r
            try {
              r = await fetchRemains(ctx, endpoint, keyEntry.value)
            } catch (innerErr) {
              r = {
                ok: false,
                reason: 'spawn-throw',
            ...
            if (r.ok) {
              ...
              return Object.assign({ cached: false }, payload)
            }
          }
        }
        ...
        return {
          ok: false,
          reason: firstAuthFail ? 'auth' : firstCurlFail ? 'curl-failed' : 'no-success',
        }
      } catch (err) {
