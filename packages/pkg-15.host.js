// pkg-15 host: MiniMax / MiniMaxi Token Plan coding_plan/remains fetcher.
//
// Refresh strategy (since pkg-14):
// - Listen for `agent/turn-stopping` and proactively refresh the cache
//   after every agent turn closes, so the next client poll sees fresh data.
// - RPC handler still serves cached snapshots within CACHE_MS.
// - Concurrent refreshes share one inflight Promise (no duplicate fetches).
//
// Sandbox note: ctx is read-only. The trigger tag ("rpc" vs "turn-stopping")
// is passed as a function argument, never written back to ctx.
//
// Endpoint detection:
// - minimax / MiniMax           -> api.minimax.io
// - minimax-cn / minimaxi / MiniMaxi -> api.minimaxi.com
//
...
async function runRefresh(ctx, trigger) {
  try {
...
return {
  ...
  apply(ctx) {
    function startRefresh(trigger) {
      ...
      inflight = (async () => {
        try {
          const result = await runRefresh(ctx, trigger)
          if (result && result.ok) {
    ...
    const offTurnStopping = ctx.on('agent/turn-stopping', (payload) => {
      ...
      try {
        const p = startRefresh('turn-stopping')
        p.then((result) => {
          if (result && result.ok) {
